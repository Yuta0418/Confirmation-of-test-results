        ...
        <div class="contents">
            ...
            <section class="center-box">
                <div class="center-box-inner">
                    <div class="register">
                        <form method='post' action="{{ route('admin.attentionupdate') }}">
                        @csrf
                            @foreach ($errors->all() as $error)
                                <li style="list-style: none; color: red;">{{ $error }}</li>
                            @endforeach
                            <div class="form-item">
                                <label for="">患者様画面 注意事項変更</label>
                                <textarea name="comment" cols="50" rows="5" value="{{ $attention->comment }}"></textarea>
                            </div>
                            <div class="form-item">
                                <label for="">現在設定中の注意事項</label>
                                <p class="attention">{{ $attention->comment }}</p>
                            </div>
                            <div class="btn-box">
                                <button type="button" class="btn-back" onclick='location.href="{{ route('admin.patients.list') }}"' style= "color:#fff;">{{ __('戻る') }}</button>
                                <button type="submit" class="btn_pop btn-register">更新</button>
                            </div>
                        </form>
                    </div>
                </div>
