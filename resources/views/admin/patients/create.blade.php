                ...
                <div class="center-box-inner">
                    <div class="register">
                        ...
                        @csrf
                            ...
                            <div class="form-item">
                                <label for="">氏名</label>
                                <input type="text" name="name" required value="{{old('name')}}" placeholder="ヤマダ　タロウ">
                            </div>
                            <div class="form-item">
                                <label for="">生年月日（Birth：year/month/day）</label>
                                <input type="text" name="birthday" required value="{{ old('birthday') }}" placeholder="1980/01/01">
                            </div>
                            <div class="form-item">
                                <label for="">カルテ番号</label>
                                <input type="text" name="patient_id" required value="{{ old('patient_id') }}" placeholder="0000001">
                            </div>
                            <div class="form-item">
                                <label for="">検査結果</label>
                                <div class="select">
                                    <select name="results">
                                        <option value="{{ App\Enums\Result::getValue("Choice") }}" @if(old('results') == '1') selected @endif>{{ App\Enums\Result::getDescription(1) }}</option>
                                        <option value="{{ App\Enums\Result::getValue("Negative") }}" @if(old('results') == '2') selected @endif>{{ App\Enums\Result::getDescription(2) }}</option>
                                        <option value="{{ App\Enums\Result::getValue("Positive") }}" @if(old('results') == '3') selected @endif>{{ App\Enums\Result::getDescription(3) }}</option>
                                    </select>
                                </div>
                            </div>
                            ...
                            <div class="btn-box">
                                ...
                                <button type="submit" class="btn_pop btn-register">登録</button>
                            </div>
                        </form>
                    </div>
                </div>
